...
class LISA():
    def __init__(self,m=1.328,lam=905,rmax=200,rmin=1.5,bdiv=3e-3,dst=0.05,
        ...
        if atm_model=='rain':
            ...
            self.N_model = lambda D : self.Nd_fog(D)

            self.augment = lambda pc, visibility : self.augment_avg(pc, visibility)
    ...
    def augment_avg(self, pc, visibility=None):
        ...
        if visibility is not None:
            alpha = 2.996 / visibility
        else:
            alpha, beta = self.alpha_beta(Nd)  # extinction coeff. (1/m) 


        # print(f"vis = {visibility}, a = {alpha}")
