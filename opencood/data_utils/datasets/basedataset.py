...
class BaseDataset(Dataset):
    ...
    def __init__(self, params, visualize, train=True, isSim=False, operator="ORI"):
        ...
        self.train = train
        self.isSim = isSim
        self.operator = operator  # add metamorph operators to the data, "ORI" indicates no processing is done
    ...
    def retrieve_base_data(self, idx, cur_ego_pose_flag=True):
        ...
        for cav_id, cav_content in scenario_database.items():
            ...
            # add operators 'RN', 'SW', 'FG' here
            if self.operator in ['RN', 'SW', 'SG']:
                data_meta = DataMetamorph(self.operator, cav_content[timestamp_key]['lidar'])
                data_meta.execute()
            # add communication latency operator here ('CT')
            elif self.operator == 'CT':
                data_meta = DataMetamorph(operator=self.operator, data_path=cav_content[timestamp_key]['lidar'], is_ego=cav_id)
                timestamp_delay = data_meta.execute()

            data[cav_id] = OrderedDict()
            data[cav_id]['ego'] = cav_content['ego']

            # print(timestamp_delay)
            ...
            # # TODO: record data path
            data[cav_id]['timestamp'] = timestamp_key_delay

            # # add flag for operators 'CL' and 'GL' here
            # if self.operator in ['CL', 'GL']:
            #     data[cav_id]['operator'] = self.operator
    ...
    def reform_param(self, cav_content, ego_content, timestamp_cur,
        ...
        # add spatial misalignment operator here ('SM')
        if not cav_content['ego'] and self.operator == 'SM':
            delay_data_meta = DataMetamorph(operator=self.operator, pose_matrix=delay_cav_lidar_pose)
            cur_data_meta = DataMetamorph(operator=self.operator, pose_matrix=cur_cav_lidar_pose)
            delay_cav_lidar_pose = delay_data_meta.execute()
            cur_cav_lidar_pose = cur_data_meta.execute()
